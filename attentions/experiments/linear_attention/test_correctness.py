...
for T in SEQ_LENS:
    ...
    recurrent_outputs, recurrent_states = linear_attention_recurrent(queries, keys, values)
    parallel_outputs, parallel_states = linear_attention_parallel(queries, keys, values)

    output_diff = (recurrent_outputs - parallel_outputs).abs().max().item()
    state_diff = (recurrent_states - parallel_states).abs().max().item()

    equal = torch.allclose(recurrent_outputs, parallel_outputs, atol=2e-3, rtol=1e-4)
